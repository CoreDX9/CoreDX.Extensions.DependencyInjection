...
public static class CastleDynamicProxyDependencyInjectionExtensions
{
        ...
        params Type[] interceptorTypes)
    {
        ...
        var originalServiceDescriptor = services.LastOrDefault(service => service.IsKeyedService is false && service.ServiceType == serviceType && service.Lifetime == serviceLifetime)
            ?? throw new ArgumentException($"Not found registered \"{Enum.GetName(typeof(ServiceLifetime), serviceLifetime)}\" service of type {serviceType.Name}.", nameof(serviceType));
        ...
        params Type[] interceptorTypes)
    {
        ...
        var originalServiceDescriptor = services.LastOrDefault(service => service.IsKeyedService && service.ServiceKey == serviceKey && service.ServiceType == serviceType && service.Lifetime == serviceLifetime)
            ?? throw new ArgumentException($"Not found registered keyed(key value: {serviceKey}) \"{Enum.GetName(typeof(ServiceLifetime), serviceLifetime)}\" service of type {serviceType.Name}.", nameof(serviceType));
...
#else
            if (!(typeof(IInterceptor).IsAssignableFrom(type) || typeof(IAsyncInterceptor).IsAssignableFrom(type)))
#endif
