...
using System.Text;
using CoreDX.Extensions.DependencyInjection.ServiceLookup;
...
namespace CoreDX.Extensions.DependencyInjection
{
    ...
    internal sealed class DependencyInjectionEventSource : EventSource
    {
        ...
        private readonly List<WeakReference<TypedImplementationFactoryServiceProvider>> _providers = new();
        ...
        [NonEvent]
        public void ServiceResolved(TypedImplementationFactoryServiceProvider provider, Type serviceType)
        {
        ...
        [NonEvent]
        public void CallSiteBuilt(TypedImplementationFactoryServiceProvider provider, Type serviceType, ServiceCallSite callSite)
        {
            if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
            {
        ...
        [NonEvent]
        public void DynamicMethodBuilt(TypedImplementationFactoryServiceProvider provider, Type serviceType, int methodSize)
        {
            if (IsEnabled(EventLevel.Verbose, EventKeywords.All))
            {
        ...
        [NonEvent]
        public void ServiceProviderBuilt(TypedImplementationFactoryServiceProvider provider)
        {
            lock (_providers)
            {
                _providers.Add(new WeakReference<TypedImplementationFactoryServiceProvider>(provider));
            }
        ...
        [NonEvent]
        public void ServiceProviderDisposed(TypedImplementationFactoryServiceProvider provider)
        {
            lock (_providers)
            {
                for (int i = _providers.Count - 1; i >= 0; i--)
                {
                    ...
                    WeakReference<TypedImplementationFactoryServiceProvider> reference = _providers[i];
                    if (!reference.TryGetTarget(out TypedImplementationFactoryServiceProvider? target) || target == provider)
                    {
        ...
        [NonEvent]
        private void WriteServiceProviderBuilt(TypedImplementationFactoryServiceProvider provider)
        {
        ...
        protected override void OnEventCommand(EventCommandEventArgs command)
        {
            if (command.Command == EventCommand.Enable)
            {
                ...
                lock (_providers)
                {
                    foreach (WeakReference<TypedImplementationFactoryServiceProvider> reference in _providers)
                    {
                        if (reference.TryGetTarget(out TypedImplementationFactoryServiceProvider? provider))
                        {
                            WriteServiceProviderBuilt(provider);
                        }
                    }
    ...
    internal static class DependencyInjectionEventSourceExtensions
    {
        ...
        public static void ExpressionTreeGenerated(this DependencyInjectionEventSource source, TypedImplementationFactoryServiceProvider provider, Type serviceType, Expression expression)
        {
